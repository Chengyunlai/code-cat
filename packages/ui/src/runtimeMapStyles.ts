export const runtimeMapStyles = String.raw`
    ...
    * { box-sizing: border-box; }
    ...
    .rich-text { line-height: 1.65; white-space: normal; }
    .rich-text > :first-child { margin-top: 0; }
    .rich-text > :last-child { margin-bottom: 0; }
    body[data-host="jetbrains"] .product-name, body[data-host="jetbrains"] .title-separator { display: none; }
    body[data-host="jetbrains"] .app-header { padding: 7px 14px; }
    body[data-host="jetbrains"] .composer-shortcut { display: none; }
    .rich-text p { margin: 0 0 10px; white-space: pre-wrap; }
    .rich-text h3 { margin: 20px 0 8px; font-size: 14px; font-weight: 600; text-wrap: balance; }
    .rich-text ul, .rich-text ol { margin: 8px 0 10px; padding-left: 22px; }
    .rich-text li { white-space: normal; }
    ...
    .action.source-reference { appearance: none; display: inline; min-height: 0; border-radius: 3px; padding: 0; border: 0; background: transparent; color: var(--cc-accent); font: inherit; text-decoration: underline; text-underline-offset: 3px; cursor: pointer; }
    .action.source-reference:hover { color: var(--cc-accent); background: var(--cc-accent-wash); text-decoration-thickness: 2px; }
    .source-reference:focus-visible { outline: 2px solid var(--cc-accent); outline-offset: 2px; }
    .code-block { margin: 14px 0; overflow: hidden; border: 1px solid var(--vscode-panel-border); border-radius: 7px; }
    .code-block-header { display: flex; align-items: center; justify-content: space-between; gap: 12px; padding: 5px 10px; background: var(--cc-surface); font-size: 11px; }
    .code-block .action { padding: 3px 7px; font-size: 11px; }
    .rich-text .code-block pre { margin: 0; border-radius: 0; line-height: 1.65; }
    .history-debug-actions { display: flex; flex-wrap: wrap; align-items: center; gap: 8px 14px; margin-top: 12px; font-size: 12px; }
    .stream-status { color: var(--vscode-descriptionForeground); font-size: 11px; margin-top: 10px; }
