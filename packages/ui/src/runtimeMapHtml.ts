...
export function createRuntimeMapHtml(webview: { readonly cspSource: string }): string {
...
<body>
    ...
    <footer class="composer">
      <div class="composer-shell">
        <div class="composer-toolbar">
          <div id="primary-debug-action"></div>
        <details class="tools-menu" id="tools-menu">
        <summary aria-label="更多调试与阅读操作">更多</summary>
        <div class="tools-panel">
        <div id="debug-controls" class="debug-controls"></div>
        <div id="evidence-context" class="evidence-context"></div>
        ...
        <div id="pause-rail" class="pause-rail shell" aria-label="暂停历史" hidden></div>
        ...
        <div id="composer-inner" class="composer-inner">
          ...
          <div class="composer-meta">
            ...
            <div class="composer-controls">
              <button id="cancel-question" type="button" class="action quiet" hidden>停止回答</button>
  ...
  <template id="debug-actions">
    <div class="action-row">
      <button type="button" class="action primary" data-debug="stepOver" title="执行当前行并在下一处暂停（Step Over），验证你的判断">单步验证</button>
      <button type="button" class="action" data-debug="stepInto" title="单步进入当前行调用的函数（Step Into）">进入函数</button>
      <button type="button" class="action" data-debug="continue" title="继续运行，直到下一个断点或程序结束">继续运行</button>
      <button type="button" class="action quiet" data-action="explain">解释现场</button>
    </div>
  </template>
